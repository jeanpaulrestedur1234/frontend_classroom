  ...
  return (
    <Card className="space-y-5 relative overflow-hidden">
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-zinc-500">
        <span>
          <span className="text-zinc-600">{t('myPackages.createdAt')}:</span>{' '}
          {formatDate(pkg.created_at)}
        </span>
        {pkg.activated_at && (
          <span>
            <span className="text-zinc-600">{t('myPackages.activatedAt')}:</span>{' '}
            {formatDate(pkg.activated_at)}
          </span>
        )}
        {pkg.expires_at && (
          <span>
            <span className="text-zinc-600">{t('myPackages.expiresAt')}:</span>{' '}
